...
@bot.event
async def on_ready():
    logger.info("Ready! Username: {}, ID: {}.".format(
        bot.user.name, bot.user.id))
    await bot.change_presence(activity=discord.Game(
        name="Type {}help".format(bot.command_prefix)))
...
async def on_command_error(ctx, exception):
    ...
    logger.error("Message: \"{0}\" by user {1} with ID {1.id}".format(
        ctx.message.content, ctx.message.author))
...
extensions = ('cogs.utility', 'cogs.fun', 'cogs.members',
              'cogs.admin', 'cogs.config', 'cogs.server')
